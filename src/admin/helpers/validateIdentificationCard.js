...
export const isValidIdentificationCard = (ic) => {
    var isNumeric = true;
    var total = 0, 
        individual;	
    ...
    for (var position = 0 ; position < 10 ; position++) {
        ...
        individual = ic.toString().substring(position, position + 1)

        if(isNaN(individual)) {
            // console.log(ic, position,individual, isNaN(individual))
            isNumeric=false;				
            break;			
        } else {
    ...
    if(isNumeric) {	
        ...
        // console.log(ic, total, individual);
        // console.log(ic, typeof ic, ic.length)
        // La cédula debe contener al menos 10 dígitos
        if(ic.toString().length != 10) { 
